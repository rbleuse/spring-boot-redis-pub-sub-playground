...
describe('JobStore', () => {
  ...
  it('does not erase newer live state when a REST seed arrives later', () => {
    const event: JobProgressEvent = {
      jobId: 'j1',
      name: 'resize',
      status: 'RUNNING',
  ...
  it('preserves live-only jobs when seeding REST snapshots', () => {
    const event: JobProgressEvent = {
      jobId: 'jX',
      name: 'etl',
      status: 'QUEUED',
    ...
    store.apply(event);
    store.seed([job]);
    expect(store.jobs().map((j) => j.jobId)).toEqual(['jX', 'j1']);
  });
});
